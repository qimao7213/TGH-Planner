...
std::shared_ptr<GazeboInfoProcess> global_gazebo_info_process = nullptr;
...
class Nodelet : public nodelet::Nodelet {
 private:
  ...
  void timer_callback(const ros::TimerEvent& event) {
    ...
    transform_broadcaster.sendTransform(tf::StampedTransform(transform,
                                                              ros::Time::now(), "/world",
                                                              "/base_link"));
    global_gazebo_info_process->pubRobotState(odom_msg);
 ...
 public:
  void onInit(void) {
    ...
    nh.param("simulator/Diff_Model", use_teb_, false);
    ...
    global_gazebo_info_process = std::make_shared<GazeboInfoProcess>(nh);
  }
};
}  // namespace car_simulator
