...
Astar2D::~Astar2D() {
  ...
  // TODO: 把z限制住
int Astar2D::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt, double margin,
  ...
  while (!open_set_.empty()) {
    ...
    if (reach_end) {
      // cout << "[Astar2D]:---------------------- " << use_node_num_ << endl;
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = 0.0; dz <= 0.0 + 1e-3; dz += resolution_) {
          d_pos << dx, dy, dz;
          ...
          //                         -1.0);
          double dist = edt_environment_->evaluateCoarseEDT(pro_pos, -1.0, 1);
          if (dist <= margin_) {
            continue;
          }
          ...
          if (pro_node == NULL) {
            ...
            if (use_node_num_ == allocate_num_) {
              // cout << "Astar 2D run out of memory." << endl;
              return NO_PATH;
            }
...
void Astar2D::setParam(ros::NodeHandle& nh) {
  nh.param("search_2D/resolution_astar", resolution_, 0.1);
  nh.param("search_2D/time_resolution", time_resolution_, 0.8);
  nh.param("astar_2D/lambda_heu", lambda_heu_, 1.2);
  nh.param("astar_2D/margin", margin_, 0.3);                    //其实这个就是cleance_吧
  nh.param("astar_2D/allocate_num", allocate_num_, 1000);
  tie_breaker_ = 1.0 + 1.0 / 10000;

  cout << "margin:" << margin_ << endl;
}

void Astar2D::retrievePath(NodePtr end_node) {
...
std::vector<Eigen::Vector3d> Astar2D::getPath() {
...
// 这个应该对2D的同样适用，只要我把扩展的点的z都卡为同一个值
double Astar2D::getDiagHeu(const Eigen::Vector3d& x1, const Eigen::Vector3d& x2) {
...
double Astar2D::getManhHeu(const Eigen::Vector3d& x1, const Eigen::Vector3d& x2) {
  double dx = fabs(x1(0) - x2(0));
  double dy = fabs(x1(1) - x2(1));
  double dz = fabs(x1(2) - x2(2));
...
double Astar2D::getEuclHeu(const Eigen::Vector3d& x1, const Eigen::Vector3d& x2) {
...
void Astar2D::setEnvironment(const EDTEnvironment::Ptr& env) {
...
std::vector<NodePtr> Astar2D::getVisitedNodes() {
  vector<NodePtr> visited;
  visited.assign(path_node_pool_.begin(), path_node_pool_.begin() + use_node_num_ - 1);
  return visited;
}

Eigen::Vector3i Astar2D::posToIndex(const Eigen::Vector3d& pt) {
...
int Astar2D::timeToIndex(double time) {
