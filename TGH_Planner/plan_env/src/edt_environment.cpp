...
void EDTEnvironment::getSurroundDistance2D(Eigen::Vector3d pts[2][2], double dists[2][2]) {
  for (int x = 0; x < 2; x++) {
    for (int y = 0; y < 2; y++) {
        dists[x][y] = sdf_map_->getDistance2D(pts[x][y]);
    }
  }
}
...
void EDTEnvironment::interpolateTrilinear2D(double values[2][2],
                                            const Eigen::Vector3d& diff,
                                            double& value,
                                            Eigen::Vector3d& grad) {
  // trilinear interpolation
  ...
  grad[2] = (v1 - v0) * resolution_inv_;
  grad[1] = ((1 - diff[2]) * (v10 - v00) + diff[2] * (v11 - v01)) * resolution_inv_;
  grad[0] = (1 - diff[2]) * (1 - diff[1]) * (values[1][0] - values[0][0]);
  grad[0] += (1 - diff[2]) * diff[1] * (values[1][1] - values[0][1]);
  grad[0] += diff[2] * (1 - diff[1]) * (values[1][0] - values[0][0]);
...
double EDTEnvironment::evaluateCoarseEDT(Eigen::Vector3d& pos, double time, bool only2D) 
{
  double d1;
  if(only2D)
  {
    d1 = sdf_map_->getDistance2D(pos);
  }
  else
  {
    d1 = sdf_map_->getDistance(pos);
  }
...
void EDTEnvironment::evaluateEDTWithGrad2D(const Eigen::Vector3d& pos,
                                           double time, double& dist,
                                           Eigen::Vector3d& grad) {
  Eigen::Vector3d diff;
  Eigen::Vector3d sur_pts[2][2];
  sdf_map_->getSurroundPts2D(pos, sur_pts, diff);
  diff(2) = 0;
  double dists[2][2];
  getSurroundDistance2D(sur_pts, dists);

  interpolateTrilinear2D(dists, diff, dist, grad);
}

double EDTEnvironment::getMinDisViaLineSeg2D(const Eigen::Vector3d &pt1, 
                                                    const Eigen::Vector3d &pt2)
{
    double currDis;  
    Eigen::Vector3d currGrad;
    Eigen::Vector3d half = Eigen::Vector3d(0.5, 0.5, 0.5);
    double minDis;        
    this->evaluateEDTWithGrad2D(pt2, -1, minDis, currGrad);

    Eigen::Vector3d ray_pt;    
    raycasterLineCheck_.setInput(pt1 / sdf_map_->getResolution(), pt2 / sdf_map_->getResolution());
    while (raycasterLineCheck_.step(ray_pt)) {
        Eigen::Vector3d tmp = (ray_pt + half) * sdf_map_->getResolution();
        this->evaluateEDTWithGrad2D(tmp, -1, currDis, currGrad);
