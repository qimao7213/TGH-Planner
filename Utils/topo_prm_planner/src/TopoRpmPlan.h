...
public:
    ...
    void init(ros::NodeHandle& nh)
    {
        ...
        sdf_map_->initMapFromCostMap(); // 这里订阅了地图
        ...
        last_goal_ = Eigen::Vector3d(-1000, -1000, -1000);
        last_start_ = Eigen::Vector3d(-1000, -1000, -1000);
        std::string record_file_path;
        nh.param<std::string>("record_file_path", record_file_path, "xx/record_data.txt");

        record_file_.open("record_file_path", std::ios::out);
    ...
    void run()
    {
        while (listener_->newMsg())
        {
            ...
            if((start_state - last_start_).norm() > 5) 
            {
                topo_prm_->resetPathContainer(); 
                start_change.clear();
                ROS_WARN("Start position changed too much, reset the path container!");
            }
                       
            if((goal_state - last_goal_).norm() > 0.25) 
            {
                topo_prm_->resetPathContainer();
                ROS_WARN("Goal position changed too much, reset the path container!");
            ...
            std::cout << "topo_prm_->findVoroPaths start!" << std::endl;
            ...
            if(!path_back.empty()) visualization_->drawTopoPathsPhase1(path_back, 0.1);
            if(!path_front.empty()) visualization_->drawTopoPathsPhase2(path_front, 0.1);
            ...
            auto voro_paths = topo_prm_->getVoroPathsForPub();
            if(!voro_paths.empty()) visualization_->drawTopoVoronoiPaths(voro_paths, 0.2);

            last_goal_ = goal_state; last_start_ = start_state;
...
private:
    ...
    Eigen::Vector3d last_goal_, last_start_;
