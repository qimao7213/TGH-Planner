...
void DynamicVoronoi::visualize(const char* filename)
{
  ...
  for (int y = sizeY - 1; y >= 0; --y)
  {
    ...
    {
      unsigned char c = 0;
      if (alternativeDiagram != NULL && (alternativeDiagram[x][y] == free 
          || alternativeDiagram[x][y] == voronoiKeep || alternativeDiagram[x][y] == voronoiVertex))
      {
        fputc(255, F);
        fputc(0, F);
        fputc(0, F);
...
void DynamicVoronoi::updateAlternativePrunedDiagram()
{
  ...
  while (!sortedPruneQueue.empty())
  {
    ...
    {
      if(alternativeDiagram[p.x][p.y] != voronoiVertex)
      {
  ...
  while (!end_cells.empty())
  {
    ...
    {
      if(alternativeDiagram[p.x][p.y] == voronoiVertex) continue;
...
int DynamicVoronoi::getNumVoronoiNeighborsAlternative(int x, int y) const
{
  ...
  for (int dx = -1; dx <= 1; ++dx)
  {
    for (int dy = -1; dy <= 1; ++dy)
    {
      ...
      if (alternativeDiagram[nx][ny] == free || alternativeDiagram[nx][ny] == voronoiKeep || alternativeDiagram[nx][ny] == voronoiVertex)
      {
        ++count;
      }
    }
