...
public:
  ...
  // points：控制点，order：阶数pb，interval：ts
  // 均匀生成节点向量，虽然前几个值为负数，但是不影响
  // points是从混合A*结果里面按照时间均匀采样出来的路径点得到的，所以用这些控制点生成均匀B样条也是合理的
  ...
  double getLength(const double& t_start, const double& t_end,
                                    const double& res = 0.01);
  double getJerk();
  void   getMeanAndMaxVel(double& mean_v, double& max_v);
  void   getMeanAndMaxAcc(double& mean_a, double& max_a);

  double calCurvature(const double& u);

  vector<double> getCurvature(double dt = 0.01);
  void recomputeInit();

  //  
  Eigen::Matrix4d M_matrix_;
  Eigen::VectorXd evaluateDeBoorT_test(const double& t);
  Eigen::VectorXd evaluateDeBoor_test(const double& u);
  void getControlPointIdxAndWeight(vector<int>& idx, vector<double>& weights, const double& ts);
