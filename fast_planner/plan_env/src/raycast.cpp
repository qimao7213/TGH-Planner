...
bool RayCaster::step(Eigen::Vector3d& ray_pt, bool verbose) {
  ...
  if(verbose) 
  {
    std::cout << "start_: " << start_.transpose() << ", end_: " << end_.transpose() 
                        << ", current: " << Eigen::Vector3i(x_, y_, z_).transpose();
  
    std::cout << ". tMaxX_: " << tMaxX_ << ", tMaxY_: " << tMaxY_ << ", tMaxZ_: " << tMaxZ_ << std::endl;
  }

  if ((x_ == endX_ && y_ == endY_ && z_ == endZ_) 
      //  || abs(tMaxX_) >= 1.1 &&  abs(tMaxY_) >= 1.1 &&  abs(tMaxZ_) >= 1.1
      // 这里不是tMaxX_大于1算是遍历完毕，而是tMaxX_遍历了 1 之后，算是遍历完毕
     ) {
    return false;
  }

  // if ((Eigen::Vector3d(x_, y_, z_) - start_).squaredNorm() > maxDist_ + 10)
  // {
  //   return false;
  // }
