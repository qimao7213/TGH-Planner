...
int ObjHistory::queue_size_ = 8;//这个相当于是滑窗，这个是什么地方初始化的呀？
int ObjHistory::skip_num_ = 0;
ros::Time ObjHistory::global_start_time_;
// vector<ros::Publisher> pose_pubs_;  // obj pose (from optitrack)
ros::Publisher obj_pub_;            // visualize marker
bool pred_success_ = false;
...
void ObjPredictor::init() {
  /* get param */
  node_handle_.param("prediction/obj_num", obj_num_, 10);
  node_handle_.param("prediction/lambda", lambda_, 1.0);//和什么加速度调节有关
  node_handle_.param("prediction/predict_rate", predict_rate_, 1.0);//预测频率，单位hz
  obj_pub_ = node_handle_.advertise<visualization_msgs::Marker>("/dynamic/obj_pred", 10);
  // for (int i = 0; i < obj_num_; ++i) {
  //   ros::Publisher pose_pub =
  //       node_handle_.advertise<geometry_msgs::PoseStamped>("/dynamic/pose_pred_" + std::to_string(i), 10);
...
void ObjPredictor::predictCallback(const ros::TimerEvent& e) {
  // predictPolyFit();
  predictConstVel();
  for(int i = 0; i < obj_num_; ++i) visualizeObj(i);
}

// 把所有obj的cube尺寸记录下来，如果全部都记录了，就关闭这个sub
...
void ObjPredictor::visualizeObj(int id) {
  if(!pred_success_) return;
  
  Eigen::Vector3d pos, color, scale;
  pos = getPositionAtTime(id, 1.0);
  // std::cout << "pos_pred:" << pos.transpose() << std::endl;
  scale = (*obj_scale_)[id];
  double yaw = 0.0;
  ...
  mk.color.a = 0.5, mk.color.r = 0, mk.color.g = 0, mk.color.b = 0;
